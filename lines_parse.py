...
def process_training_lines(lines, voca, throw_id=True, delim=','):
    ...
    for l in lines:
        for indx, f in enumerate(l):
            mean_l[indx] += float(f)
            sum_l[indx] += 1
    min_l = [float(feature) for feature in l0]
    max_l = [float(feature) for feature in l0]
    range_l = [0.0 for feature in l0]
    for l in lines:
        for i, f in enumerate(l):
            max_l[i] = max(max_l[i], float(f))
            min_l[i] = min(min_l[i], float(f))
    ...
    for i, l in enumerate(lines):
        for j, f in enumerate(l):
            if range_l[j] != 0:
                lines[i][j] = (float(lines[i][j]) / mean_l[j]) / range_l[j]
    if throw_id:
...
def load_data_and_split(file_name, skip_header=True, delim=','):
    ...
    testing_lines = [reduce(lambda x, y: x + ',' + y, l[:-1]) for l in test_lines]
